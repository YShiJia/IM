...
func (d *fileDAO) PreloadFileSlices() func(db *gorm.DB) *gorm.DB {
	return func(db *gorm.DB) *gorm.DB {
		return db.Preload("FileSlices")
	}
}

func (d *fileDAO) GetByHash(hash string, conds ...func(db *gorm.DB) *gorm.DB) (*model.File, error) {
	if hash == "" {
		return nil, fmt.Errorf("invalid hash")
	}
	f := &model.File{}
	if err := d.getTx().Model(&model.File{}).
		Scopes(conds...).
		Where(&model.File{Hash: hash}).
